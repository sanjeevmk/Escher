...
def interpolate(src_mesh:Mesh, tgt_mesh:Mesh, interval:float, transformation_method="tetrahedralize",fragment_resolution="quadratic") -> List[Mesh]:
    """
    ...
    if transformation_method=="tetrahedralize":
        ...
        if fragment_resolution == "average":
            ...
            for t in np.arange(0,1+interval,interval):
                ...
                interpolated_mesh = Mesh(vertices=new_vertices,faces=src_mesh.faces,process=False)
            ...
            for t in np.arange(0,1+interval,interval):
                ...
                interpolated_mesh = Mesh(vertices=interpolated_vertices,faces=src_mesh.faces,process=False)
                interpolated_meshes.append(interpolated_mesh)

    elif transformation_method=="one_ring":
        src_one_ring_batch = src_mesh.get_vertices_as_one_ring_matrices()
        tgt_one_ring_batch = tgt_mesh.get_vertices_as_one_ring_matrices()

        per_vertex_target_scales = []
        per_vertex_slerp_instances = []
        per_vertex_target_translations = []
        per_vertex_target_rotations = []

        identity_rotation = np.expand_dims(np.eye(3),0)

        for i,src_ring in enumerate(src_one_ring_batch):
            tgt_ring = tgt_one_ring_batch[i]
            assert src_ring.shape[1]==3, logging.error("The one-ring matrix should be a Nx3 matrix")

            src_ring_center_vertex = np.expand_dims(src_mesh.vertices[i],0)
            tgt_ring_center_vertex = np.expand_dims(tgt_mesh.vertices[i],0)

            #below function calls requires 3xN point sets, hence transpose each
            translation = alg.get_translation_for(src_ring - src_ring_center_vertex,
                                                    tgt_ring - tgt_ring_center_vertex)

            mat_Q = alg.get_transformation_matrix_for((src_ring - src_ring_center_vertex).T,
                                                        (tgt_ring  - tgt_ring_center_vertex).T)
            R,S = alg.get_rotation_scale_from_transformation(mat_Q)

            rotation_endpoints_matrix = np.concatenate([identity_rotation,np.expand_dims(R,0)],axis=0)

            _slerp = Slerp(times=[0,1],rotations=Rotation.from_matrix(rotation_endpoints_matrix))
            per_vertex_target_scales.append(S)
            per_vertex_target_rotations.append(R)
            per_vertex_slerp_instances.append(_slerp)
            per_vertex_target_translations.append(translation.squeeze())

        per_vertex_target_scales = np.array(per_vertex_target_scales)
        per_vertex_target_translations = np.array(per_vertex_target_translations)
        ...
        new_faces = []
        interpolated_meshes = []
        for _index in range(len(src_mesh.vertices)):
            t = 0.9
            interpolated_rotation_matrix_vertex = per_vertex_slerp_instances[_index]([t])[0].as_matrix()
            interpolated_scale_matrix_vertex = (1-t)*np.eye(3) + t*per_vertex_target_scales[_index]
            interpolated_transformation_matrix = interpolated_rotation_matrix_vertex @ interpolated_scale_matrix_vertex

            src_vertex = np.expand_dims(src_mesh.vertices[_index],-1)
            new_vertex = (interpolated_transformation_matrix @ src_vertex).T #+ per_vertex_target_translations[_index]

            new_vertices.extend(new_vertex.tolist())
        ...
        for t in np.arange(0,1+interval,interval):
            interpolated_vertices = []
            interpolated_translations = t*per_vertex_target_translations
            for point_index in range(len(src_mesh.vertices)):
                vertex = src_mesh.vertices[point_index]
                vertex_interpolated_rotation = per_vertex_slerp_instances[point_index]([t])[0]
                vertex_rotation_matrix = vertex_interpolated_rotation.as_matrix()
                vertex_scale_matrix = (1-t)*np.eye(3) + t*per_vertex_target_scales[point_index]
                interpolated_vertex = ((vertex_rotation_matrix @ vertex_scale_matrix) @ np.expand_dims(vertex,0).T).T
                interpolated_vertex +=  interpolated_translations[point_index]
                interpolated_vertices.append(interpolated_vertex.squeeze())

            #interpolated_transformation = interpolated_rotation_matrices # @ interpolated_scale_matrices).tolist()
            #interpolated_block_transformation = block_diag(*interpolated_transformation)
            #interpolated_vertices_unrolled = interpolated_block_transformation @ src_vertices_unrolled
            #interpolated_vertices_rot_scale = np.reshape(interpolated_vertices_unrolled,(-1,3))


            #interpolated_vertices = interpolated_vertices_rot_scale + interpolated_translations
